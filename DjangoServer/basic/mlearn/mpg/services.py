...
class MpgService:
    ...
    def __init__(self):
        global mpg_origin
        mpg_origin = pd.read_csv(os.path.join(os.getcwd(), 'data/mpg.csv'))
    '''
    1.스펙보기
    '''
    def spec(self):
        print(" --- 1.Shape ---")
        print(mpg_origin.shape)
        print(" --- 2.Features ---")
        print(mpg_origin.columns)
        print(" --- 3.Info ---")
        print(mpg_origin.info())
        print(" --- 4.Case Top1 ---")
        print(mpg_origin.head(1))
        print(" --- 5.Case Bottom1 ---")
        print(mpg_origin.tail(3))
        print(" --- 6.Describe ---")
        print(mpg_origin.describe())
        print(" --- 7.Describe All ---")
        print(mpg_origin.describe(include='all'))
    ...
    def rename_meta(self):
        global mpg
        mpg = mpg.rename(columns=mpg_meta)
        print(" --- 2.Features ---")
        print(mpg.columns)
    ...
    def visualize(self): # No.8
        mpg['종합연비'] = (mpg['시내주행'] + mpg['고속주행'])/2
        mpg['종합연비'] = np.where(mpg['종합연비']>=20, 'pass', 'fail')
        mpg['종합연비'].value_counts().plot.bar(rot=0)
        plt.savefig('./save/bar_graph.png')
    '''
    4.배기량이 4이하와 5이상 자동차의 고속주행연비 비교 (p.144)
    '''
    def compare_displ(self):
        print(f"배기량이 4 이하 고속주행: {mpg.query('배기량 <= 4')['고속주행'].mean():.2f}" )
        print(f"배기량이 5 이상 고속주행: {mpg.query('배기량 >= 5')['고속주행'].mean():.2f}")
    '''
    5.시내주행 연비 평균이 가장 높은 회사는? (p.150)
    '''
    def find_high_cty(self):
        print(f"아우디의 평균 도시연비: %0.2f"% mpg.query('회사 == "audi"')['시내주행'].mean())
        print(f"토요타의 평균 도시연비: %0.2f"% mpg.query('회사 == "toyota"')['시내주행'].mean())
    '''
    6.고속주행 연비 평균이 가장 높은 회사는? (p.150)
    '''
    def find_highest_hwy(self):
        all_avg = mpg.query('회사 in ["chevrolet","ford","honda"]')
    ...
    def which_cty_in_suv_compact(self):
        print("suv 자동차의 시내주행: %0.2f"%
              mpg.query("차종 == 'suv'")['시내주행'].mean())  # clazz가 suv인 행만 추출한 다음 cty 호출 후 그 객체의 평균구함
        print("compact 자동차의 시내주행: %0.2f"% mpg.query("차종 == 'compact'")['시내주행'].mean())
