...
from multiplex.path import HAAR, static
from matplotlib import pyplot as plt
from mosaic import ImageToNumberArray, Hough, Haar, mosaic, Canny, mosaics
...
def Hough(param):
    ...
    return dst
def Haar(param):
    haar = cv.CascadeClassifier(static+HAAR)
    if param.size == 0:
        print("### param is null ### ")
    else :
...
class MenuController(object):
    ...
    def menu_5(*param):
        print(param[0])
        cat = cv.imread(static+param[1])
        mos = mosaic(cat, (150, 150, 450, 450), 10)
        cv.imwrite(static+cat-mosaic.png, mos)
